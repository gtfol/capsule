...
function InfoTooltipContent({ label, children }: Omit<Props, "active">) {
  ...
  return <Tooltip.Provider delayDuration={200}><Tooltip.Root open={open} onOpenChange={setOpen}>
    <Tooltip.Trigger asChild><button
      ...
      aria-label={label}
      aria-expanded={open}
      className="-my-1 inline-flex size-6 shrink-0 items-center justify-center text-subtle transition-colors hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground"
