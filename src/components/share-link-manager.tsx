...
export function ShareLinkManager({ space, onBack }: { space: string; onBack: () => void }) {
  ...
  return <>
    ...
    {loading ? <p className="mt-5 flex items-center gap-2 text-[12px] text-muted-foreground" role="status"><Loader2 size={13} className="animate-spin" aria-hidden="true" />Opening links…</p> : entries.length ? <ul className="mt-3 max-h-[min(440px,50dvh)] divide-y divide-border overflow-y-auto">{entries.map((entry) => <li key={entry.record.id} className="py-4">
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="truncate text-[12px]" title={titleOf(entry)}>{titleOf(entry)}</p><p className="mt-1 text-[11px] text-subtle">{entry.record.pending ? "Creation unconfirmed" : entry.record.expiresAt === null ? "Never expires" : `Expires ${new Date(entry.record.expiresAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}`}{!entry.record.pending && entry.record.views !== undefined && <> · {entry.record.views === 1 ? "1 view" : `${entry.record.views} views`}</>}</p></div>
