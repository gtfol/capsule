...
function ShareSession({ space, targetKey, target, appearance }: { space: string; targetKey: string; target: ShareTarget; appearance: "nav" | "icon" }) {
  ...
  const [busy, setBusy] = useState<"create" | "update" | "expiry" | "remove" | null>(null);
  ...
  async function mutate(action: NonNullable<typeof busy>) {
    ...
    setBusy(action); setError(""); setStatus(""); setCopied(false);
    try {
  ...
  if (managing) return <PopoverContent align={appearance === "nav" ? "center" : "end"} side={appearance === "nav" ? "top" : "bottom"} sideOffset={8} className={panelClass} aria-label="Manage share links"><ShareLinkManager space={space} onBack={() => { setManaging(false); setLoading(true); setError(""); setStatus(""); setReload((value) => value + 1); }} /></PopoverContent>;
  ...
  return <PopoverContent align={appearance === "nav" ? "center" : "end"} side={appearance === "nav" ? "top" : "bottom"} sideOffset={8} className={panelClass} aria-labelledby={titleId} aria-describedby={descriptionId}>
    ...
    {loading ? <p role="status" className="mt-5 flex items-center gap-2 text-[12px] text-muted-foreground"><Loader2 size={13} className="animate-spin" aria-hidden="true" />Checking share link…</p> : <>
      ...
      {confirmed && <div className="mt-5">
        <div className="flex items-center gap-3 border-b border-border pb-2"><input ref={linkInput} type="text" readOnly value={url} aria-label="Share link" onFocus={(event) => event.currentTarget.select()} className="min-w-0 flex-1 bg-transparent py-1 text-[12px] outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground" /><button type="button" className="shrink-0 p-1 text-muted-foreground hover:text-foreground" onClick={() => void copyLink()} aria-label={copied ? "Link copied" : "Copy share link"}>{copied ? <Check size={15} aria-hidden="true" /> : <Copy size={15} strokeWidth={1.5} aria-hidden="true" />}</button></div>
      ...
      {enabled && <>
        ...
        {expiry === "never" && <p className="mt-2 text-[11px] leading-relaxed text-subtle">Available until you remove the link.</p>}
        <p className="mt-3 text-[11px] leading-relaxed text-subtle">Manage this link from this browser.</p>
        ...
        {!confirmed ? <Button type="button" className="mt-5 w-full" disabled={!!busy || !canPublish} onClick={() => void mutate("create")}>{busy === "create" ? <><Loader2 size={13} className="animate-spin" aria-hidden="true" />Creating link…</> : record?.pending ? "Retry creating link" : "Create link"}</Button> : <>
          {changed && <p className="mt-4 text-[11px] text-subtle">Saved changes are ready to share.</p>}
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3"><Button type="button" variant="ghost" className="text-[12px]" disabled={!!busy || !canPublish} onClick={() => void mutate("update")}>{busy === "update" ? <><Loader2 size={13} className="animate-spin" aria-hidden="true" />Updating…</> : "Update link"}</Button>{expiry !== confirmed.expiry && <Button type="button" variant="ghost" className="text-[12px]" disabled={!!busy} onClick={() => void mutate("expiry")}>{busy === "expiry" ? "Saving expiry…" : "Save expiry"}</Button>}</div>
        </>}
        {record && <button type="button" className="mt-5 text-[12px] text-muted-foreground hover:text-foreground disabled:opacity-40" disabled={!!busy} onClick={() => void mutate("remove")}>{busy === "remove" ? "Removing link…" : "Remove link"}</button>}
      </>}
    </>}
    ...
    {(target.kind === "wardrobe" || target.kind === "wishlist") && <button type="button" disabled={!!busy} onClick={() => setManaging(true)} className="mt-5 border-t border-border pt-4 text-[12px] text-muted-foreground hover:text-foreground disabled:opacity-40">Manage links</button>}
  </PopoverContent>;
}
