...
function WardrobeSurface() {
  const [{ view, to, import: importUrl }, setNavigation] = useQueryStates(navigationParsers, navigationOptions);
  ...
  return <div className="app-ready min-h-dvh bg-background text-foreground"><a className="skip-link" href="#main">Skip to wardrobe</a>
    ...
    {selected && <ItemDetail key={selected.id} active={view === "wardrobe"} item={selected} onClose={() => setSelected(null)} onSave={async (item) => { if (useWardrobe.getState().space !== space) throw new Error("The active wardrobe changed. Open this piece again to save."); await saveItem(item); }} onDelete={remove} />}
  </div>;
}
