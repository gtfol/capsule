...
function WardrobeSurface() {
  const [{ view, to }, setNavigation] = useQueryStates(navigationParsers, navigationOptions);
  ...
  const setView = (target: CapsuleView) => { if (view !== target) void setNavigation(navigationQuery(target)); };
  const openAdd = (target: AddDestination) => { if (view !== "add" || to !== target) void setNavigation(navigationQuery("add", target)); };
