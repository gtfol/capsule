...
export function Outfits({ onAdd }: { onAdd: () => void }) {
  ...
  return <section className="outfits-view" aria-labelledby="outfits-heading"><div className="outfits-head"><h1 id="outfits-heading" className="text-[13px] font-semibold">Outfits <span className="ml-3 text-[11px] font-normal text-neutral-400">{String(outfits.length).padStart(2, "0")}</span></h1>{items.length > 0 && outfits.length > 0 && <Button variant="ghost" disabled={busy} onClick={() => setCreating(!creating)} className="text-[12px]">{creating ? "Back to outfits" : <><Plus size={13} />Create outfit</>}</Button>}</div>
    {!items.length && !outfits.length ? <div className="empty-state"><p className="font-medium">No pieces yet.</p><p className="mt-2 text-[12px] text-neutral-400">Add pieces to your wardrobe to create an outfit.</p><button className="mt-7 inline-flex items-center gap-3 border-b border-black pb-1 text-[12px]" onClick={onAdd}>Add a piece<ArrowRight size={13} /></button></div> : builder ? <form onSubmit={render} className="outfit-workspace">
      <div><div className="flex items-center justify-between"><h2 className="text-[12px] font-medium">Choose pieces</h2><span className="text-[11px] text-neutral-400">{selectedItems.length} / 6</span></div><div className="outfit-picker">{items.map((item) => <button key={item.id} type="button" className="outfit-piece" aria-label={`Select ${item.name}`} aria-pressed={selection.includes(item.id)} disabled={busy || (!selection.includes(item.id) && selection.length >= 6)} onClick={() => toggle(item.id)}>
      ...
      <div className="space-y-6"><div><div className="mb-4 flex items-center justify-between"><h2 className="text-[12px] font-medium">Your reference photo</h2>{referencePhoto && <button type="button" className="text-[11px] text-neutral-400" disabled={busy} onClick={() => { void setReferencePhoto(null).catch(() => setError("The reference photo could not be removed.")); }}>Remove</button>}</div>{referencePhoto ? <div>
        {/* eslint-disable-next-line @next/next/no-img-element */}
    ...
    {detail && <Sheet open onOpenChange={(open) => { if (!open) setDetail(null); }}><SheetContent><SheetTitle className="text-[13px] font-semibold">{detail.name}</SheetTitle><SheetDescription className="sr-only">Rendered outfit and its wardrobe pieces.</SheetDescription>
      {/* eslint-disable-next-line @next/next/no-img-element */}
