...
type PhotoState = { choices: PhotoChoice[]; frontId: string; backId: string | null };
...
function initialPhotos(item: ItemDraft | Item, images: string[], uploadedImages: string[]): PhotoState {
  if (uploadedImages.length) {
    ...
    return { choices, frontId: choices[0].id, backId: choices[1]?.id ?? null };
  }
  ...
  return { choices: choices.slice(0, 24), frontId: choices[0]?.id ?? "", backId: hasBack ? "back" : null };
}
...
export function ItemDetail({ item, images = [], uploadedImages = [], isNew = false, onClose, onSave, onDelete }: Props) {
  ...
  const [photos, setPhotos] = useState(() => initialPhotos(item, images, uploadedImages));
  ...
  const selectedId = imageSide === "front" ? photos.frontId : photos.backId;
  ...
  function selectImage(id: string) {
    ...
    setBackgroundError("");
    setPhotos((current) => {
      if (imageSide === "front") return { ...current, frontId: id, backId: current.backId === id ? current.frontId : current.backId };
      if (id === current.frontId) return current.backId ? { ...current, frontId: current.backId, backId: id } : current;
      return { ...current, backId: id };
    });
  }
  ...
  function close() { if (saving) return; cancelBackground(); cancelPhotoWork(); onClose(); }
  ...
  async function addPhotos(files: File[]) {
    ...
    setPhotoError(""); setPhotoStatus("");
    if (files.length > 2) { setPhotoError("Choose one or two photos at a time."); return; }
    const token = ++photoSequence.current;
    ...
    try {
      ...
      setPhotoStatus("Photos added. Choose a front or back image.");
    } catch (cause) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const { imageData, backImageData } = await cacheProductImages(chosenImage(front), back ? chosenImage(back) : undefined);
      const now = Date.now();
      await onSave({ ...form, name: form.name.trim(), purchaseUrl, imageUrl: front.imageUrl, imageData, backImageUrl: back?.imageUrl, backImageData, id: "id" in item ? item.id : crypto.randomUUID(), createdAt: "createdAt" in item ? item.createdAt : now, updatedAt: now, deletedAt: null } as Item);
      onClose();
  ...
  return <Sheet open onOpenChange={(open) => { if (!open) close(); }}><SheetContent data-busy={saving} onEscapeKeyDown={(event) => { if (saving) event.preventDefault(); }} onInteractOutside={(event) => { if (saving) event.preventDefault(); }}>
    ...
    {photos.choices.length > 1 && <div className="mt-7 flex items-center justify-between gap-4">
      ...
      {photos.backId && <button type="button" disabled={imageControlsLocked} className="text-[11px] text-muted-foreground underline underline-offset-4" onClick={() => { setPhotos((current) => ({ ...current, backId: null })); setImageSide("front"); setBackgroundError(""); }}>Remove back</button>}
    </div>}
    <div className="detail-image mt-6 flex aspect-[5/4] items-center justify-center bg-background" data-cutout={review ? review.preview === "cutout" : !!selectedPhoto?.useCutout}>
      ...
      {previewSource ? <img src={previewSource} alt={`${form.name || "Product image"}, ${imageSide}`} className="h-full w-full object-contain" /> : <p className="text-[12px] text-subtle">Choose a back image below. Optional.</p>}
    </div>
    {photos.choices.length > 1 && <div className="mt-4 flex gap-3 overflow-x-auto pb-2" aria-label="Product images">{photos.choices.map((choice, i) => <button type="button" key={choice.id} aria-label={`Use image ${i + 1} as ${imageSide}`} aria-pressed={selectedId === choice.id} disabled={imageControlsLocked || (imageSide === "back" && choice.id === photos.frontId && !photos.backId)} className={`relative h-14 w-12 shrink-0 border-b disabled:opacity-40 ${selectedId === choice.id ? "border-foreground" : "border-transparent"}`} onClick={() => selectImage(choice.id)}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
    ...
    {backgroundProgress ? <div className="background-tools" aria-live="polite"><div className="flex items-center gap-2 text-[12px]"><Loader2 size={13} className="animate-spin" /><span>{backgroundProgress.stage === "download" ? "Downloading model…" : "Removing background…"}{backgroundProgress.progress === undefined ? "" : ` ${Math.round(Math.max(0, Math.min(100, backgroundProgress.progress)))}%`}</span></div><button type="button" className="text-[12px] underline underline-offset-4" onClick={cancelBackground}>Cancel</button></div> : review ? <div className="background-review">
