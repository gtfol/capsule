...
import { ArrowRight, Check, Link2Off, Plus, X } from "lucide-react";
...
export function Wishlist({ onAdd, onMoved, active = true, pieceId, onPieceClosed }: { pieceId?: string | null; onPieceClosed?: () => void; onAdd: () => void; onMoved: () => void; active?: boolean }) {
  const { wishlist, space, updateWishlistItem, deleteWishlistItem, saveWishlistItem, moveWishlistToWardrobe } = useWardrobe();
  const [category, setCategory] = useState<Category | "all">("all");
  const [sort, setSort] = useState<Sort>("recent");
  const [filterOpen, setFilterOpen] = useState(false);
  const [selectedPiece, setSelected] = useState<WishlistItem | null>(null);
  const selected = selectedPiece ?? (pieceId ? wishlist.find((item) => item.id === pieceId) ?? null : null);
  ...
  return <>
    ...
    <section className="catalog-content" aria-label="Wishlist pieces"><div className="catalog-heading gap-4"><h1>Wishlist{category !== "all" ? ` / ${CATEGORIES.find((entry) => entry.value === category)?.label}` : ""}</h1><div className="flex shrink-0 items-center gap-4 sm:gap-6"><span className="text-[11px] text-subtle">{String(filtered.length).padStart(2, "0")}</span><button type="button" onClick={onAdd} className="text-muted-foreground hover:text-foreground" aria-label="Add to wishlist"><Plus size={14} strokeWidth={1.5} /></button></div></div>
      {filtered.length ? <div className="product-grid">{filtered.map((item) => <button type="button" className="product-card" key={item.id} onClick={() => { onPieceClosed?.(); setSelected(item); }} aria-label={`View ${item.name}`}><ProductImages item={item} /><span className="product-name">{item.name}</span><span className="product-meta">{formatPrice(item.price, item.currency)}</span><span className="mt-2 flex w-full items-center justify-between gap-3"><StarRating value={item.rating} compact />{item.link_broken && <span title="A listing could not be fetched" aria-label="Link unavailable"><Link2Off size={13} strokeWidth={1.5} className="text-muted-foreground" /></span>}</span></button>)}</div> : <div className="empty-state"><p>{wishlist.length ? "No pieces in this category." : "Your wishlist is empty."}</p><p className="mt-2 text-[12px] text-subtle">Add a piece with a link or photos.</p><button className="mt-7 inline-flex items-center gap-3 border-b border-foreground pb-1 text-[12px]" onClick={onAdd}>Add to wishlist<ArrowRight size={13} /></button></div>}
    </section>
