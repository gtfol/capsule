...
export function SharedCapsule({ shareId, snapshot, updatedAt, expiresAt }: { shareId: string; snapshot: ShareSnapshot; updatedAt: number; expiresAt: number | null }) {
  ...
  return <div className="min-h-dvh bg-background text-foreground">
    ...
    <main id="shared-main" className={collection ? "catalog-layout" : "mx-auto max-w-5xl px-[30px] pb-28 pt-[30px] max-sm:px-5"}>
      {collection ? <aside className="catalog-sidebar" aria-label="Shared collection filters"><Link href="/" className="mb-6 block w-fit text-[12px]">capsule</Link><div className="category-list" role="tablist" aria-label="Categories"><button role="tab" aria-selected={category === "all"} onClick={() => setCategory("all")} className={category === "all" ? "category active" : "category"}>ALL</button>{CATEGORIES.map((value) => <button key={value} role="tab" aria-selected={category === value} onClick={() => setCategory(value)} className={category === value ? "category active" : "category"}>{value.toUpperCase()}</button>)}</div></aside> : <Link href="/" className="mb-6 block w-fit text-[12px]">capsule</Link>}
    ...
    <nav className="bottom-nav" aria-label="Shared capsule"><div className="bottom-nav-inner"><Link href="/" className="nav-button">Create your capsule</Link><span className="nav-dot" aria-hidden="true">•</span><ThemeControl /></div></nav>
