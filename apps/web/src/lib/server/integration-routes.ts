...
export async function integrationRequest(request: Request, kind: "lookup" | "wishlist" | "wardrobe" | "purchase" | "update-wishlist" | "update-wardrobe" | "delete-wardrobe" | "wardrobe-item" | "wishlist-item" | "delete-wishlist" | "wishlist-price", id?: string, view?: string) {
  try {
    handlers ??= createIntegrationHandlers(getPool());
    return await (kind === "lookup" ? handlers.lookup(request) : (kind === "wardrobe-item" || kind === "wishlist-item") ? handlers.wardrobeItem(request,id!,view,kind === "wishlist-item" ? "wishlist" : "items") : handlers.write(request,kind,id));
