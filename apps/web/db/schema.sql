...
create table if not exists public.capsule_integration_tokens (
  ...
  scopes text[] not null check (cardinality(scopes) > 0 and scopes <@ array['items:read','wishlist:write','wardrobe:write','wardrobe:delete','wishlist:delete','outfits:read','outfits:write','outfits:delete']::text[]),
  created_at timestamptz not null default now(),
  expires_at timestamptz,
  revoked_at timestamptz,
